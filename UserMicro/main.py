...
@app.post('/user/register/', status_code=status.HTTP_201_CREATED, response_model=UserCreated, tags=['user'])
async def create_user(user_request: User):
    user = await create_user_in_db(user_request=user_request)
    return user


@app.get('/user/user/', status_code=status.HTTP_200_OK, response_model=UserCreated, tags=['user'])
async def get_logged_user(user: TokenData = Depends(get_current_user)):
    user = await get_user_by_id(user_id=user.id)
    return user


@app.post('/user/login/', status_code=status.HTTP_200_OK, tags=['auth'], response_model=Token)
